...
def optimize_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"メモリ使用量: {original_memory:.2f}MB → {optimized_memory:.2f}MB "
...
def verify_data_integrity(csv_path: Path, parquet_path: Path, sample_size: int = 10000) -> bool:
    """データ整合性をチェック"""
    print(f"\n=== データ整合性チェック ===")
    ...
    print(f"  列数: CSV={len(csv_sample.columns)}, Parquet={len(parquet_sample.columns)}")
    ...
    for col in numeric_cols[:5]:  # 最初の5列をチェック
        ...
        max_error = np.max(np.abs(csv_values - parquet_values) / (np.abs(csv_values) + 1e-10))
        if max_error > 1e-6:
...
def benchmark_loading_speed(csv_path: Path, parquet_path: Path, iterations: int = 3) -> Dict[str, float]:
    """読み込み速度をベンチマーク"""
    print(f"\n=== 読み込み速度ベンチマーク ===")
...
def main():
    ...
    for filename, result in results.items():
        print(f"\n📄 {filename}:")
        print(f"  ファイルサイズ削減: {result['original_size_mb']:.1f}MB → {result['converted_size_mb']:.1f}MB")
    ...
    print(f"\n📊 全体サマリー:")
    print(f"  総ファイルサイズ: {total_original_size:.1f}MB → {total_converted_size:.1f}MB")
