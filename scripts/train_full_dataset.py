...
def prepare_features(train_df: pd.DataFrame, test_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, List[str]]:
    """特徴量を準備"""
    print("\n=== 特徴量準備 ===")
    ...
    if y.dtype == 'O':  # object型（文字列）の場合
        ...
        print(f"クラスマッピング: {dict(zip(le.classes_, le.transform(le.classes_)))}")
...
def train_random_forest(X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray) -> Tuple[RandomForestClassifier, Dict[str, Any]]:
    """RandomForestモデルを訓練"""
    print("\n=== RandomForest訓練 ===")
...
def train_lightgbm(X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray) -> Tuple[lgb.LGBMClassifier, Dict[str, Any]]:
    """LightGBMモデルを訓練"""
    print("\n=== LightGBM訓練 ===")
...
def evaluate_model(model, X_val: np.ndarray, y_val: np.ndarray, class_names: List[str] = None) -> Dict[str, Any]:
    """詳細なモデル評価"""
    y_pred = model.predict(X_val)
...
def create_submission(model, X_test: np.ndarray, test_df: pd.DataFrame, algorithm_name: str) -> str:
    """提出ファイルを作成"""
...
def plot_results(results: List[Dict[str, Any]], output_dir: Path):
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'full_dataset_comparison.png', dpi=300, bbox_inches='tight')
    plt.show()


def save_results(results: List[Dict[str, Any]], feature_cols: List[str], output_dir: Path):
    """結果を保存"""
    # メタデータを追加
    summary = {
...
def main():
    ...
    try:
        ...
        print(f"📈 ベースライン比較: {baseline_accuracy:.4f} → {best_model['accuracy']:.4f}")
